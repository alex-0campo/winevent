...
"@
    ...
    #start of test code
    #Get-WinEvent -Path $($lastArchivedLog.FullName) -FilterXPath $xpath
    #end of test code


    if ( $lastArchivedLogLastWriteTime -ge $((Get-Date).AddHours(-1)) )
    {
...
$ms = $(New-TimeSpan -Minutes $searchRangeMinutes).TotalMilliseconds # events in the past 15-minutes(s)
...
do
{
    $reportDate = Get-Date
    $reportDate
    $failedEventGroups = Get-FailedEvents | group id | sort count -Descending
    ...
    Write-Host "Pause for $((New-TimeSpan -Minutes $sleepMinutes).TotalSeconds) seconds`r`n" -ForegroundColor Red
    Start-Sleep -Seconds $((New-TimeSpan -Minutes $sleepMinutes).TotalSeconds) # pause for 1-minute
} while( $true )
