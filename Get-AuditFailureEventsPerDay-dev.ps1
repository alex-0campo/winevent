...
# simulate execution every 1-hour
# do {
    ...
    6. Get audit failures every hour compared to previous hours.
    ...
    #########################################################################################
    $searchRangeMinutes = 1440 # 24hrs * 60min/hour
    ...
    $sleepDuration = 3600000 # 3600000 # (n) seconds = 1-hour
    ...
    $lastArchivedLogs = Get-ChildItem -Filter "Archive-ForwardedEvents*.evtx" -Path $searchPath | Select -ExpandProperty FullName -Last 4 | sort LastWriteTime -desc
    ...
    "Report Range: {0} -to- {1}" -f $start.ToLocalTime(), $start.AddMinutes($searchRangeMinutes).ToLocalTime()
    "`r`n"
...
"@
    ...
    $xpath = "*[System[band(Keywords,4503599627370496) and TimeCreated[@SystemTime >= $strStart and @SystemTime <= $strEnd]]]"
...
"@
    ...
    #region filterxpath attributes ###  
        ...
        # $xpath = "*[System[TimeCreated[@SystemTime) >= $strStart and @SystemTime <= $strEnd]]]"
    #endregion
    ...
    #region Get-WinEvent
    $lastArchivedLogs | ForEach-Object {
        Write-Host "Searching events from $_`r`n"
        $failedEvents += Get-WinEvent -Path $_ -FilterXPath $xpath -ErrorAction Stop
    }
    ...
    $failedEventsGroup | ForEach-Object {
        ...
        $_.Group | group MachineName | ForEach-Object {
            ...
            if ($_.Count -ge $eventCountThreshold)
            {
                ...
                $sbBody.Append($(($_.Name).Split(".")[0]) + " reported " + $($_.Count) + " event IDs " + $($_.Group.Id | Select -First 1 -Unique) + " (failures) in the past 15-minutes.<br />") | Out-Null
                # $sbBody.ToString()
